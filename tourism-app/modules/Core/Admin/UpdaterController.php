...
class UpdaterController extends  AdminController
{
    ...
    public function index(){
        ...
        $data = [
            'ready_for_update'=>(setting_item('one2one_license_key') and setting_item('one2one_username')) ? true : false
        ];
    ...
    public function checkUpdate(){
        ...
        if(!setting_item('one2one_license_key') or !setting_item('one2one_username'))
        {
        ...
        $data = file_get_contents_curl($url,true,[
            'one2one_license_key'=>setting_item('one2one_license_key'),
            'one2one_username'=>setting_item('one2one_username'),
            'action'=>'check_update',
            'domain'=>\request()->getHost(),
            'product'=>'TanzaniaTourism',
        ]);
    ...
    public function doUpdate(){
        ...
        if(empty($updater_latest_version) or version_compare(config('app.version'),$updater_latest_version,'>=')){
            return $this->sendError(__("You are using latest version of Tanzania Tourism"));
        }
    ...
    public function storeLicense(){
        ...
        \request()->validate([
            'one2one_license_key'=>'required',
            'one2one_username'=>'required'
        ]);

        Settings::store('one2one_license_key',trim(\request()->input('one2one_license_key')));
        Settings::store('one2one_username',trim(\request()->input('one2one_username')));
