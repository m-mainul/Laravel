...
//        event(new VendorLogPayment($booking));
    ...
    public function addRequestQuote(Request $request){
        ...
        if(Auth::check()) {
            $user = Auth::user();
            $row->name = $user->first_name .' '.$request->input('last_name');
            $row->phone = $user->phone;
            $row->email = $user->email;
        }
        $row->default_person_adult = $request->input("default_person_adult");
        $row->default_person_child = $request->input("default_person_child");
        $row->ea_person_adult = $request->input("ea_person_adult");
        $row->ea_person_child = $request->input("ea_person_child");
        $row->tz_person_adult = $request->input("tz_person_adult");
        $row->tz_person_child = $request->input("tz_person_child");
...
//        event(new EnquirySendEvent($row));
    ...
    public  function checkAuthenticateUserRequestQuote(Request $request){
        if(request()->ajax()) {
            if(!is_enable_guest_checkout() and !Auth::check()){
                return $this->sendError(__("You have to login in to do this"))->setStatusCode(401);
            }
            ...
            return response()->json(['message'=>'Authenticated User', 'success'=>true]);
        }
    }
