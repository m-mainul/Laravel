...
<div class="modal-body">
    <form action="{{ route('workLogStore', array('project_id' => $work_status->project_id,'user_id'=>$work_status->user_id)) }}" method="post" enctype="multipart/form-data" class="form-horizontal" id="task_pause">
        ...
        <div class="form-group">
            ...
            ?>
            <label class="col-sm-12 control-label">Allocated Time is: {{ (int)($assn_minutes/60) }} Hours {{ $assn_minutes%60 }} Minutes</label> 
        </div>
        ...
        <div class="form-group">
            ...
            ?>
            <label class="col-sm-8 control-label">Time Remaining: @if ($remaining_mins < 0) {{"Time Over"}} @else {{ (int)($remaining_mins/60) }}:{{ $remaining_mins%60 }} @endif</label> 
        </div>

        <div class="form-group">
            <label class="col-sm-8 control-label">Deadline: {{ date("D M d", strtotime($work_status->next_deadline)) }}</label> 
        </div>
        
        @if ((($user_role_id == 3) && ($autheticate_user == 1) && ($work_status->status == 'started')))
        <div class="form-group">
            <label for="log_hour" class="col-sm-4 control-label">Log Hour: </label>
            <div class="col-sm-6">
        ...
        @if (($user_role_id == 3) && ($autheticate_user == 1))
        ...
        <div class="col-sm-6">
            ...
            <a href="{{ route('projectStatusChange',array('project_id' => $work_status->project_id,'user_id'=>$work_status->user_id)) }}"
            ...
            <button type="submit" name = "task_paused" class="btn btn-default" 
            ...
            <br>Pause</button>
            @if ($work_status->status == 'started')
             {{-- <button type="button" name = "task_done" class="btn btn-default"> --}}
                <a href="{{ route('toProjectDone',array('project_id' => $work_status->project_id,'user_id'=>$work_status->user_id)) }}" id="task_done" class="btn btn-default"><span class="glyphicon glyphicon-off"></span><br>Done</a>
             {{-- </button> --}}
            @endif
        </div>
        </div>
