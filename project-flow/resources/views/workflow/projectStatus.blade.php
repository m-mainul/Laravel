...
<div class="modal-body">
    <form action="{{ route('workLogStore', array('id'=>$work_status->id, 'project_id' => $work_status->project_id,'user_id'=>$work_status->user_id)) }}" method="post" enctype="multipart/form-data" class="form-horizontal" id="designer-form">
        ...
        <div class="form-group">
            ...
            <label class="col-sm-7 control-label" id="est_start_time">{{ date("D M d h:i", strtotime($work_status->est_start_time)) }}</label>
        </div>
        @endif
        
        <div class="form-group" style="margin-bottom: 10px;">
            <label class="col-sm-5 control-label">Deadline: </label> 
            <label class="col-sm-7 control-label">{{ date("D M d h:i", strtotime($work_status->next_deadline)) }}</label>
        </div>

        @if (($user_role_id == 3) && ($work_status->status == 'started') && ($authenticate_user == 1))
        <div class="form-group log-hour">
            <label for="log_hour" class="col-sm-3 control-label">Log Hour: </label>
            <div class="col-sm-4">
            ...
            <div class="error-msg" style="display: inline-block; margin-left: 250px">
        ...
        @elseif ($user_role_id == 2)
            <div class="form-group" style="margin: 20px auto; text-align: center;">
               <div class="col-sm-12">
                   <a href="{{ route('toEditBrief', $work_status->id) }}"  class="btn btn-default brief-edit" id="brief-edit" data-toggle="modal" data-target="#briefEditModal" data-remote="false">Edit</a>
                  <button type="button" class="btn btn-default brief-delete" id="brief-delete" data-url="{{route('toDeleteBrief',$work_status->id)}}">Delete</button>
               </div>
           </div>
