...
<div class="modal-body">
    <form action="{{ route('workLogStore', array('project_id' => $work_status->project_id,'user_id'=>$work_status->user_id)) }}" method="post" enctype="multipart/form-data" class="form-horizontal" id="task_pause">
        ...
        <div class="col-sm-12">
            ...
            <a href="{{ route('projectStatusChange',array('project_id' => $work_status->project_id,'user_id'=>$work_status->user_id, 'brief_number' =>$work_status->brief_number)) }}"
            ...
            @if ($work_status->status == 'started')
             {{-- <button type="submit" name = "task_done" class="btn btn-default"> --}}
                <a href="{{ route('toProjectDone',array('project_id' => $work_status->project_id,'user_id'=>$work_status->user_id, 'brief_number' => $work_status->brief_number)) }}" id="task_done" class="btn btn-default"><span class="glyphicon glyphicon-off"></span><br>Done</a>
             {{-- </button> --}}
            @endif
        </div>
        </div>
